...
"build cat12: cat cat1 cat2\n");
  ...
  /// State of command_runner_ and logs contents (if specified) ARE MODIFED.
  /// Handful to check for NOOP builds, and higher-level rebuild tests.
...
"  depfile = out.d\n";
  ...
  // Check that disavalidating deps by target timestamp also works here
  // Repeate the test but touch target instead of blanking the log.
  fs_.Tick();
  fs_.Create("header.in", "");
  fs_.Create("out", "");
  RebuildTarget("out", manifest, "build_log", "ninja_deps2");
